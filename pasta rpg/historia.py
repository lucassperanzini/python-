...
def item_7():
    print(''' Antes que você tenha tempo de chegar a uma porta, o rochedo já está sobre você, que grita de dor e medo quando ele o esmaga no chão. 
...
def item_8():
    print('''
O Demônio do Espelho agarra-o pelo pulso. Imediatamente, ele começa a puxá-lo na direção do espelho.
A força dele é incrível, e, apesar de todos os seus esforços, você não consegue evitar que o arraste progressivamente na direção do espelho.
Quando ele toca o espelho, parece desaparecer diretamente através dele. Horrorizado, você vê seu próprio braço desaparecer, seguido pelo resto do corpo.
Você está agora em um mundo de espelhos de outra dimensão, do qual jamais retornará.
''')

def item_9():
    print('''
Os Hobgoblins não têm nada que lhe seja útil, por isso você resolve abrir o saco no chão.
Dentro, acha uma moringa de barro arrolhada. Você a desarrolha e cheira o líquido que contém.
O odor é penetrante e acre.
Se quiser beber um pouco do líquido, vá para 158. Se quiser mergulhar um pedaço de pano nele, vá para 375.
''')
    

def item_10():
    print('''
Ainda correndo o mais rápido possível, você enfia a mão na mochila e tira o tubo de madeira.
Seu plano é ficar sob a água, respirando pelo tubo. Com sorte, os Trogloditas pensarão que você será arrastado até a morte rio abaixo, pois a torrente desaparece nas profundezas da montanha.
Você segura o tubo entre os dentes e mergulha na água. Segurando-se em um dos pilares da ponte embaixo d'água, você fica absolutamente imóvel por 10 minutos.
Quando finalmente acha que os Trogloditas foram embora, você sobe à superfície e olha em volta. Não há ninguém à vista, e você sai do rio e atravessa a ponte para a margem norte.
Quaisquer Provisões restantes que você possa ter estão agora encharcadas e imprestáveis. Risque-as de sua Folha de Aventuras.
Você segue pela vasta caverna até que, finalmente, vê um túnel na parede do outro lado.
Você caminha até uma pesada porta de madeira, que está trancada.
Se você tiver uma chave de ferro, vá para 86. Se não tiver, vá para 276.
''')
    

def item_11():
    print('''
Você olha para baixo e vê os corpos esparramados dos Guardiões Voadores imóveis no chão.
Então, começa a forçar o olho de esmeralda do ídolo para retirá-lo com a ponta da espada.
Finalmente, ele se solta, e você fica surpreso com o peso da jóia. Esperando que possa ser útil mais tarde, você o coloca na mochila.
Se quiser agora retirar o olho direito, vá para 140. Se preferir descer do ídolo, vá para 46.
''')
...
Se responder corretamente, ficará livre - mas, se sua resposta estiver errada, eu o transformarei em pedra!"
Ele volta a dar risadinhas, obviamente feliz com sua chegada.
Você:
...
def item_13():
    print('''
O túnel faz uma curva abrupta para a esquerda e se dirige para o norte, tanto quanto a sua vista alcança.
As pegadas que você está seguindo começam a sumir à medida que o túnel vai ficando mais seco.
Logo não vê mais o teto gotejante e as poças no chão.
Você repara que o ar está se tornando mais quente, e se vê ofegante, embora esteja andando bem devagar.
Em uma pequena reentrância da parede da esquerda, você vê um pedaço de bambu na vertical.
Levantando-o, repara que ele está cheio de um líquido claro.
...
def item_15():
    print('''
Uma sensação de cócegas desce pela sua espinha enquanto você se arrasta cuidadosamente para fora do aposento.
De volta ao túnel, você solta um suspiro de alívio e fecha a porta com força.
Feliz com sua boa sorte, parte para o oeste mais uma vez. Vá para 74.
''')
    

def item_16():
    print('''
Você só teve tempo de ouvir o Gnomo dizer: "Três crânios",
antes que um raio branco de energia disparasse da fechadura 
e o atingisse no peito, derrubando-o inconsciente. Jogue um dado,
some 1 ao número obtido e reduza este total da sua ENERGIA. 
Se você ainda estiver vivo, recupera a consciência e o Gnomo manda 
que tente de novo. Você escolheu as gemas erradas da outra vez; portanto
não tentará aquela combinação novamente.
...
Jogue dois dados. Se o total for igual ou menor que sua HABILIDADE, vá para 55. Se o total for maior que sua HABILIDADE, vá para 202
''')
...
def item_20():
    print('''
Somente sua incrível força poderia resistir à mordida da aranha venenosa. Contudo, você está enfraquecido e repara que sua mão está tremendo ao colocar a Peça de Ouro no bolso.
Reduza sua HABILIDADE em 1 ponto. Você amaldiçoa a pessoa que largou a mochila e parte para o norte de novo. Vá para 279.
''')
...
def item_24():
    print('''
Colocada em uma alcova em arco na parede do túnel, você vê uma cadeira de madeira ornamentada, esculpida na forma de uma ave de rapina de aparência demoníaca.
Se você quiser se sentar na cadeira, vá para 324. Se preferir continuar seguindo para o norte, vá para 188.
''')
...
def item_26():
    print('''
A pílula faz com que você se sinta mole e letárgico. Você perde 2 pontos de HABILIDADE. O Anão diz que agora você pode prosseguir para o segundo estágio do teste.
Ele pega uma cesta de vime e lhe diz que ela contém uma serpente. Vira a cesta e a serpente cai no chão. É uma naja, e se ergue no ar, pronta para dar o bote. O Anão diz que pretende testar suas reações. Você deverá agarrar a naja, com as mãos nuas, pelo pescoço, evitando as presas mortais.
Você se agacha, tensionando os músculos para o momento decisivo. Jogue dois dados. Se o total for igual ou menor que sua HABILIDADE, vá para 55. Se o total for maior que sua HABILIDADE, vá para 202.
''')
...
def item_28():
    ...
    print('''
Você caminha até o apavorado homem e corta a corrente com sua espada. 
Ele cai de joelhos e se inclina, agradecendo, repetidamente. 
Diz que, há quatro anos, entrou na Prova dos Campeões, mas fracassou.
Ele caiu em um poço e teve que ser resgatado por um Juiz da Prova, 
um dos administradores do calabouço do Barão Sukumvit. Foi-lhe, então,
oferecida a opção de morrer ou servir como lacaio no Calabouço da Morte. 
Para sobreviver, ele trabalhou como escravo, até que não pôde mais suportar
e tentou escapar. Lástima, não teve êxito e foi capturado pelos Orcas, os 
...
def item_29():
    print('''
O túnel conduz ao norte por alguma distância, antes de chegar a um beco sem saída. 
A entrada de um escorrega se projeta da parede leste do túnel. Parece ser a única maneira de sair. Você resolve se arriscar e sobe no escorrega. Deslizando suavemente, você desce em um aposento, onde aterrissa de costas. Vá para 90.
''')
...
def item_64():
    print('''Logo que você põe o anel no dedo, todo seu corpo começa a tremer. Jogue dois dados. Se o total for igual ou menor que o seu índice de HABILIDADE, vá para 115. Se o total for maior que o seu índice de HABILIDADE, vá para 190.''')


def item_65():
    print('''Você bebeu uma Poção encontrada dentro de um livro de couro preto? Se beber, vá para 345. Se não, vá para 372.''')

def item_66():
    print('''
Depois de caminhar pelo túnel por alguns minutos, você chega a uma encruzilhada. Uma seta branca pintada na parede aponta para o oeste. No chão, você vê pegadas molhadas, 
feitas por aqueles que entraram antes de você. É difícil ter certeza, mas parece que três deles seguiram a direção da seta, enquanto um resolveu ir para o leste. Se você quiser ir para o oeste, vá para
...
Jogará seu escudo por cima do poço e depois pulará?	Vá para 271 Pulará por cima do poço carregando todas as suas posses?	Volte para 30 Usará a espada para trazer a corda até você, de modo a poder
usá-la para atravessar até a outra margem?	Vá para 212
...
“Se você encontrar o Mantécora, É bom de sua cauda cuidar.
Proteja-se dos espinhos Que irão voar pelo ar.”
...
def item_88():
    print('''
Logo que os TROGLODITAS o vêem pegam os arcos e correm para cercá-lo. Para seu horror, o líder dá um passo adiante e declara que você é prisioneiro deles e terá que se submeter a uma prova, segundo o rito milenar, a Corrida da Flecha. Se você estiver disposto a participar da Corrida da Flecha, vá para 343. Se preferir tentar lutar para fugir, vá para 268.
''')
...
def item_95():
    print('''
O anel de ferro está preso a um pequeno alçapão. É fácil puxá-lo, e, dentro de um pequeno compartimento, você encontra um escudo, finamente trabalhado, feito do mais puro ferro. Maravilhado com o esplendor da peça, você a amarra ao seu braço. Acrescente 1 ponto de HABILIDADE. Você caminha na direção das portas duplas e as empurra. Vá para 248.
''')
...
Em uma plataforma de pedra na parede do túnel, você vê dois livros empoeirados encadernado sem couro. Throm expressa seu desprezo pela palavra escrita com um grunhido, insistindo para que você deixe os livros de lado e siga adiante com ele. Você:
...
def item_330():
    print('''As rações do Ninja são modestas mas bem-vindas. Acrescente 1 ponto de ENERGIA. Se você ainda não o fez, poderá esfregar um pouco do unguento nos seus ferimentos – volte para 269 – ou sair do salão, levando só o diamante – volte para 127.''')

def item_331():
    print('''Tocar o pergaminho tem precisamente o efeito que você temia. O esqueleto dá um impulso para frente e, levantando-se da cadeira numa série de movimentos aos arrancos, ergue a espada para golpeá-lo. Esquivando-se para o lado, você desembainha a sua espada para se defender.
...
def item_339():
    print('''Quando você toca a maçaneta da porta, ela fica mole na sua mão, e, quando tenta tirar a mão, descobre que ela está grudada na maçaneta. Então, um punho gigantesco se forma no meio da porta e projeta-se na sua direção, atingindo-o no estômago. Você perde 1 ponto de ENERGIA. Se tiver uma moringa de ácido, volte para 303. Se não tiver, volte para 236.''')

def item_340():
    print('''O medo lhe dá uma nova injeção de energia, e, de alguma forma, suas pernas cansadas conseguem mantê-lo à frente do rochedo. Adiante, à direita, você vê a forma bem-vinda de uma porta. Você mergulha de encontro à porta e, por sorte, ela se abre. O rochedo passa estrondoso, e você fica deitado, exausto, no chão de um aposento grande. Vá para 381.''')

def item_341():
    print('''Um homem aleijado, com cadeias nos pés, arrasta-se na sua direção carregando uma bandeja de madeira cheia de pão e água. Ele parece cansado e desalentado, e tenta passar por você sem demonstrar reação à sua presença. Você:
...
def item_347():
    print('''O Anão sacode a cabeça, dizendo: “Só músculos, sem inteligência, não bastam para conquistar a Prova dos Campeões. Sinto dizer que você fracassou. Você não terá permissão para ir embora, pois poderá revelar os segredos do calabouço para outros. Todavia, você conseguiu muito chegando tão longe, e eu o indicarei para meu servo pelos anos futuros para preparar o calabouço para os novos competidores”.''')

def item_348():
    print('''Você avança sobre a Besta Sangrenta, tentando evitar a língua que se estende para agarrar-lhe a perna. Jogue dois dados. Se o total for igual ou menor que a sua HABILIDADE, volte para 225. Se o total for maior que a sua HABILIDADE, volte para 159.''')

def item_349():
    print('''Você desce pela corda para o interior do poço, segurando-se com uma das mãos, enquanto a outra leva a espada desembainhada. O Diabo do Poço é uma das feras mais terríveis que você já viu, e você sabe que esta será uma das lutas mais difíceis da sua vida.
...
def item_356():
    print('''Há uma abertura no lado esquerdo da parede do túnel. Você está de pé na entrada de uma caverna grande, quando ouve uma voz de mulher gritando por socorro. Você distingue apenas a forma de uma figura humana que rola pelo chão no fundo da caverna. Se você quiser entrar na caverna, volte para 170. Se preferir continuar para o norte pelo túnel, volte para 192.''')

def item_357():
    print('''A Besta Sangrenta chafurda na poça, e o cheiro dos gases venenosos, cujas bolhas sobem à tona e contaminam a atmosfera, provoca ânsias de vômito. Você:

Correrá contornando a poça da fera,
na direção do túnel?	Volte para 255
Jogará uma gema na poça
(se você tiver uma)?	Volte para 332
...
def item_361():
    print('''As mandíbulas do Diabo do Poço dão um bote no amuleto de macaco e o apanham no ar, mas logo se abrem de novo, forçadas pelo amuleto, que se expandiu a ponto de ocupar toda a boca da fera. Enquanto o Diabo do Poço se debate, tentando livrar-se do amuleto, você desce até o fundo para chegar às portas duplas. Desvairado, o Diabo do Poço usa o imenso corpo na tentativa de esmagar você contra a parede. Teste sua Sorte. Se você tiver sorte, volte para 82. Se não tiver sorte, vá para 377.
''')
...
def item_370():
    print('''Quando você contorna a poça correndo, a Besta Sangrenta estende a língua comprida mais uma vez. Jogue dois dados. Se o total for igual ou menor que o seu índice de HABILIDADE, volte para 104. Se o total for maior que o seu índice de HABILIDADE, volte para 342.''')

def item_371():
    print('''Você faz pontaria e joga a bola de madeira no crânio. Jogue dois dados. Se o número obtido for igual ou menor que o seu índice de HABILIDADE, volte para 273. Se o número obtido for maior que o seu índice de HABILIDADE, volte para 113.''')

def item_372():
    print('''Você finalmente chega ao corpo do guerreiro, mas, logo que toca na jóia, tanto ela quanto o guerreiro desaparecem como num passe de mágica. A porta bate atrás de você, e segue-se um estrondo agourento acima da sua cabeça. Você olha para o alto e vê o teto baixando. Corre para a porta na tentativa de escapar, mas ela está trancada e não há maçaneta do lado de dentro. O teto vai descendo, e você é obrigado a se deitar no chão, tentando impedir o movimento do teto com as mãos e os pés. Mas o esforço é inútil, e você é esmagado entre os blocos de pedra.''')
...
def item_382():
    print('''O velho aponta para uma das estátuas, e você logo a reconhece. É o cavaleiro que iniciou a Prova dos Campeões, um olhar de agonia registrado na pedra para toda a eternidade. O velho sorri, dizendo: “Este homem pesa 50 kg mais a metade do peso dele. Quanto ele pesa?” O que você responderá?
...
def item_383():
    print('''Para sua grande surpresa, nada de extraordinário lhe acontece enquanto está sentado na cadeira. Nada há a fazer senão continuar para o norte pelo túnel. Volte para 188.''')
...
def item_389():
    print('''Sem suas armas você está mais vulnerável, e a perda da espada faz com que se sinta praticamente nu. Você perde 4 pontos de HABILIDADE. Questionando se tomou a decisão correta, você segue pelo túnel para o norte. Volte para 181.''')

def item_390():
    print('''Você se agacha ao lado do pedestal, abaixo da linha de fogo das bestas. Estica a mão e puxa o crânio do pedestal, esperando que sua ação faça as bestas dispararem. Para sua grande surpresa, nada acontece. Some 1 ponto de SORTE. Ainda agachado, você arranca as jóias que formam os olhos do crânio. Você identifica as pedras amarelas - topázios - e as coloca na mochila. Olhando para a série de bestas, pergunta-se se ainda há uma armadilha à sua espera no aposento. Você:
